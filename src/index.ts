...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Bored');
}

// Original boredapi.com is dead; appbrewery's mirror at bored-api.appbrewery.com
// has different paths AND a new schema (accessibility is now a string,
// /filter returns an array, /random returns one activity object). Updated
// after the Chiark agent quality benchmark surfaced this pack at 100%
// errors.
...
const tools: McpToolExport['tools'] = [
  {
    ...
    description: 'Get a random activity suggestion to cure boredom. Returns activity name, type, participant count, and price range.',
    inputSchema: {
      type: 'object',
      properties: {},
    },
  },
  {
    ...
    description: 'Find a random activity by category (e.g., \'cooking\', \'sport\', \'relaxation\'). Returns activity name, type, participants needed, and price range.',
    inputSchema: {
      type: 'object',
      properties: {
        type: {
  ...
  },
  {
    ...
    description: 'Find a random activity for a specific group size (e.g., 1 for solo, 4 for group). Returns activity name, type, and price range.',
    inputSchema: {
      type: 'object',
      properties: {
        participants: {
...
async function randomActivity() {
  const res = await pwFetch(`${BASE_URL}/random`);
  if (!res.ok) throw await httpError(res, 'Bored API error');

  const data = (await res.json()) as BoredActivity;
  return formatActivity(data);
}

// /filter endpoints return an array — we pick one at random for the
// "give me a random X" interface this tool advertises.
async function activityByType(type: ActivityType) {
  const res = await pwFetch(`${BASE_URL}/filter?type=${encodeURIComponent(type)}`);
  if (!res.ok) throw await httpError(res, 'Bored API error');

  const data = (await res.json()) as BoredActivity[];
  const pick = pickRandom(data);
  if (!pick) throw new Error(`No activities found for type "${type}"`);
  return formatActivity(pick);
}

async function activityByParticipants(participants: number) {
  const res = await pwFetch(`${BASE_URL}/filter?participants=${participants}`);
  if (!res.ok) throw await httpError(res, 'Bored API error');

  const data = (await res.json()) as BoredActivity[];
  const pick = pickRandom(data);
  if (!pick) throw new Error(`No activities found for ${participants} participants`);
  return formatActivity(pick);
}
